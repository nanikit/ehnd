...
bool hook() {
  LPCSTR aEztFunction[] = {"J2K_Initialize",      "J2K_InitializeEx",
                           ...
                           "J2K_SetDelJPN",       "J2K_SetField",
                           "J2K_SetHnj2han",      "J2K_SetJWin",
                           "J2K_SetPriorDict",    "J2K_SetProperty",
                           "J2K_StopTranslation", "J2K_Terminate",
...
int SearchPattern(LPWORD ptn, size_t ptn_size, LPBYTE* addr) {
  ...
  while (p + ptn_size < searchEnd) {
    scan = ptnEnd;
    while (scan >= 0) {
      if ((HIBYTE(ptn[scan]) == 0x00) && (LOBYTE(ptn[scan]) != p[scan])) break;
      if (scan == 0) {
        *addr = p;
        searchSuccessCount++;
      }
...
bool hook_userdict(void) {
  ...
  if (r == 0) {
    Log(LogCategory::kNormal, L"HookUserDict : J2KEngine Pattern Search Failed\n");
    return false;
  } else if (r > 1) {
    Log(LogCategory::kNormal, L"HookUserDict : J2KEngine Pattern Search Failed\n");
    return false;
    ...
    Log(LogCategory::kNormal, L"HookUserDict : Success.\n");
  }
...
bool hook_userdict2(void) {
  ...
  if (r == 0) {
    Log(LogCategory::kNormal, L"HookUserDict2 : J2KEngine Pattern Search Failed\n");
    return false;
  } else if (r > 1) {
    Log(LogCategory::kNormal, L"HookUserDict2 : J2kEngine Pattern Search Failed\n");
    return false;
    ...
    Log(LogCategory::kNormal, L"HookUserDict2 : Success.\n");
  }
...
bool hook_getwordinfo(void) {
  ...
  */
  ...
  if (r == 0) {
    Log(LogCategory::kNormal, L"HookGetWordInfo : J2KEngine Pattern Search Failed\n");
    return false;
  } else if (r > 1) {
    Log(LogCategory::kNormal, L"HookGetWordInfo : J2kEngine Pattern Search Failed\n");
    return false;
    ...
    Log(LogCategory::kNormal, L"HookWordInfo : Success.\n");
  }
...
bool GetRealWC2MB(void) {
  ...
  if (bMatch) {
    if (*(lpfnwc2mb + 0x1E) == 0xA1) {
      LPBYTE l1, l2, l3;
      char* p = (char*)&l1;
      for (int i = 0; i < 4; i++) p[i] = *(lpfnwc2mb + 0x1F + i);
      p = (char*)&l2;
      for (int i = 0; i < 4; i++) p[i] = *(l1 + i);
      l2 += 0x15C;
      p = (char*)&l3;
      for (int i = 0; i < 4; i++) p[i] = *(l2 + i);
...
bool GetRealMB2WC(void) {
  ...
  if (bMatch) {
    if (*(lpfnmb2wc + 0x40) == 0xA1) {
      LPBYTE l1, l2, l3;
      char* p = (char*)&l1;
      for (int i = 0; i < 4; i++) p[i] = *(lpfnmb2wc + 0x41 + i);
      p = (char*)&l2;
      for (int i = 0; i < 4; i++) p[i] = *(l1 + i);
      l2 += 0x144;
      p = (char*)&l3;
...
__declspec(naked) int __stdcall MultiByteToWideCharWithAral(
